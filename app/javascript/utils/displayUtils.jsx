...
export const displayRating = rating => {
  const range = [...Array(maxRating).keys()].map(i => i + 1)

  const ratingImgs = range.map(num =>{
    const active = num <= rating ? "active" : "inactive"
    return(
      <img key={num} alt="lighting bolt" className={`rating-img img-${active}`}
