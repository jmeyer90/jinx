import React from 'react'
import { POSSIBLE_RATINGS } from './review_util'

export const displayRating = rating => {
  const ratingImgs = POSSIBLE_RATINGS.map(possibleRating =>{
    const active = possibleRating <= rating ? "active" : "inactive"
    return(
      <img key={possibleRating} alt="lighting bolt" className={`rating-img img-${active}`}
        src="data:image/png;base64," />
    )
  })

  return <span className="ratings-container">{ratingImgs}</span>
}


